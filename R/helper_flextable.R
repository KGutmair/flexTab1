...
#########################################################
# This is a helper function for the Tab1 function, which transformations a Tab1 data.frame
# into a nice flex table output
########################################################
...
helper_flextable_fun <- function(tab1,
  ...
  if (is.logical(group_var)) {
    ...
    # Calculate the sampl size stratified by group
    number_group <- data1 %>%
      group_by(group) %>%
      summarise(number = n()) %>%
    ...
    new_order <- c("name", "variable", sort(helper_col))
